...
class CatalogImagesController extends BaseController {
	 ...
	 */
	protected $layout = 'admin.catalogs.layouts.images';
  ...
  public function __construct(CatalogInterface $catalog, ImageInterface $image){
	...
	/*public function index($catalog_id)
	{
		if( Authority::can('index', 'Image') ){
			...
			$this->layout->content = View::make('admin.catalogs.images.index')->with(compact('catalog','images'));
			return $this->layout->render();
		}
		throw new NotAllowedException();
	}*/

	public function list($catalog_id)
	{
		if( Authority::can('index', 'Image') ){
			$catalog = $this->catalog->findById($catalog_id);
	...
	/*public function show($catalog_id, $id)
	{
		if( Authority::can('read', 'Image') ){
			...
			$this->layout->content = View::make('admin.catalogs.images.show', compact('catalog', 'image'));
			return $this->layout->render();
		}
		throw new NotAllowedException();
	}*/
	...
	public function edit($catalog_id, $id)
	{
		if( Authority::can('update', 'Image') ){
			$catalog = $this->catalog->findById($catalog_id);
			$image = $this->image->findByIdIn('catalog', $catalog->id, $id);
			$this->layout->content = View::make('admin.catalogs.images.edit', compact('catalog', 'image'));
			return $this->layout->render();
		}
		throw new NotAllowedException();	
	}
