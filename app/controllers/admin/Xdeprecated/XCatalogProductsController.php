...
class CatalogProductsController extends BaseController {
	 ...
	 */
	protected $layout = 'admin.catalogs.layouts.products';
  ...
  public function __construct(ProductInterface $product, CatalogInterface $catalog, ImageInterface $image){
	...
	public function index($catalog_id)
	{
		if( Authority::can('index', 'Product') ){
			...
			$this->layout->content = View::make('admin.catalogs.products.index')->with(compact('products', 'orphans', 'catalog'));
	...
	public function create($catalog_id)
	{
		...
		if( Authority::can('create', 'Product') ){
			...
			$this->layout->content = View::make('admin.catalogs.products.create', compact('catalog', 'product'));
			return $this->layout->render();
		}
		throw new NotAllowedException();
	}
	...
	public function show($catalog_id, $id)
	{
		...
		if( Authority::can('read', 'Product') ){
			...
			$this->layout->content = View::make('admin.catalogs.products.show', compact('catalog', 'product'));
			return $this->layout->render();
		}
		throw new NotAllowedException();
	}
	...
	public function edit($catalog_id, $id)
	{
		if( Authority::can('update', 'product') ){
			...
			$this->layout->content = View::make('admin.catalogs.products.edit', compact('catalog', 'product', 'imageInstance'));
			return $this->layout->render();
		}
		throw new NotAllowedException();	
	}
	...
	public function destroy($catalog_id, $id)
	{
		if( Authority::can('delete', 'Product') ){
			...
			return Redirect::route('admin.catalogs.products.index', $catalog->id)->with('success', "El producto ha sido eliminado correctamente.");
		}
		throw new NotAllowedException();
	}
	...
	public function unassign($catalog_id, $id)
	{
		if( Authority::can('delete', 'Product') ){
			...
			return Redirect::route('admin.catalogs.products.index', $catalog_id)->with('success', "El producto '$product->name' ha sido quitado correctamente.");
		}
		throw new NotAllowedException();
	}
