...
@section('buttons')
		...
		<a href="{{ route('admin.galleries.images.index', $gallery->id) }}"><button class="btn" title="Listar imagenes"><i class="icon-list"></i> Listar</button></a>
		<a href="{{ route('admin.galleries.images.edit', array($gallery->id, $image->id)) }}"><button class="btn" title="Modificar este imagen"><i class="icon-pencil"></i> Editar</button></a>
		<a class="delete" href="{{ route('admin.galleries.images.destroy', array($gallery->id, $image->id)) }}" onclick="return confirm('¿Esta seguro que desea eliminar la imagen \'{{ $image->name }}\' relacionada?')"><button class="btn" title="Eliminar imagen"><i class="icon-trash"></i> Eliminar</button></a>
@stop
			...
			<div class="page-inner">
				<dl class="dl-horizontal">
					@foreach(array(
						'name' => array( 
							'label' => 'Nombre',
							'link' => route('admin.galleries.images.edit', array($gallery->id, $image->id)),
						),
						'gallery' => array( 
							'label' => 'Galería',
							'attr' => 'name',
						...
						'status' => array(
							...
							<dt>{{ $caption }}:</dt>
								...
								@if( is_array($label) )
									...
									@if( isset( $label['attr'] ) )
										{{ HTML::link(route('admin.galleries.show', array('id' => $image->$attr->id)), $image->$attr->$label['attr'], array( 'title' => 'Ver galería' ) ) }}
									@endif
