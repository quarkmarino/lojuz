...
@section('breadcrumbs')
	...
	<li><a href="{{ route('admin.products.images.show', array($product->id, $image->id)) }}"><i class="icon-file"></i>{{ $image->name }}</a></li><span class="divider">/</span>
...
@section('buttons')
	<a href="{{ route('admin.products.images.create', $product->id) }}" onclick="return confirm('¿Esta seguro que desea salir sin guardar los detalles de la imagen?')"><button class="btn" title="Crear nuevo imagen"><i class="icon-plus"></i> Crear</button></a>
				...
				{{ Form::model($image, array('route' => array('admin.products.images.update', $product->id, $image->id), 'class' => 'af-form form-horizontal', 'id' => 'product-form', 'method' => 'put') ) }}
							...
							<div class="control-group">
								...
								<div class="controls">
									<div class="input-prepend">
										...
										{{ Form::text('product_id', $product->name, array('placeholder' => $product->name, 'class' => 'span12', 'id' => 'inputCatalog', 'disabled' => 'disabled')) }}
									</div>
								</div>
							</div>
						</div>
					...
					<div class="af-outer af-required">
						<div class="af-inner">
							...
							<a href="{{ route('admin.products.images.show', array($product->id, $image->id)) }}"><button type="button" class="btn">Cancel</button></a>
						</div>
					</div>
				{{ Form::close() }}
			</div>
