...
@section('buttons')
		...
		<a href="{{ route('admin.products.index') }}"><button class="btn" title="Listar productos"><i class="icon-list"></i> Listar</button></a>
		<a href="{{ route('admin.products.edit', $product->id) }}"><button class="btn" title="Modificar este producto"><i class="icon-pencil"></i> Editar</button></a>
			...
			<div class="page-inner">
				<dl class="dl-horizontal">
					@foreach(array(
						'name' => array( 
							'label' => 'Nombre',
							'link' => route('admin.products.edit', array('id' => $product->id)),
						),
						'catalog' => array( 
							'label' => 'Catalogo',
							'attr' => 'name',
				...
				<div class="span5">
					<div class="btn-toolbar clearfix">
						<div class="btn-group pull-right">
							<a href="{{ route('admin.products.edit', $product->id) }}"><button class="btn" title="Agregar una nueva imagen a este producto"><i class="icon-plus"></i> Agregar</button></a>
						</div>
					</div>
				</div>
			</div>
					...
					@foreach($product->images as $image)
						<article data-id="id-{{ $image->id }}" data-type="javascript html" class="span4">
							<div class="thumbnail hover-pf1">
								...
								<div class="caption">
									<h2><a class="title" href="{{ route('admin.products.images.edit', array($product->id, $image->id)) }}", title="Editar imagen"><i class="icon-pencil"></i> Ver</a></h2>
								</div>
							</div>
						</article>
						@if(++$i % 3 == 0 && $i > 0)
